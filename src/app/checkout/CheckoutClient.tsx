...
export default function CheckoutClient({
  ...
  // Stop loading indicator and scroll to top if error occurs so the user sees it on mobile
  useEffect(() => {
    if (state?.error) {
      setIsPlacing(false);
      ...
      <form onSubmit={() => setIsPlacing(true)} action={formAction}>
        ...
        <main style={s.main} className="md:!flex-row">
          <div style={s.left}>
            ...
            {state?.error && (
              <p className="text-sm text-oxblood font-semibold my-2 bg-oxblood/5 border border-oxblood/20 rounded p-3 animate-pulse">
                ⚠️ {state.error}
              </p>
            )}
