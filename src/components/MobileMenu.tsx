...
export default function MobileMenu({ session, count }: MobileMenuProps) {
  ...
  return (
    <div className="sm:hidden flex items-center">
      ...
      {/* Drawer Sidebar */}
      <div
        className={`fixed top-0 right-0 z-50 h-full w-72 max-w-[85vw] bg-cream p-6 shadow-2xl transition-transform duration-300 ease-in-out transform ${
      ...
      >
        ...
        {/* Navigation Links */}
        <nav className="flex flex-col gap-6 text-base font-semibold" style={{ fontFamily: "var(--font-stamp)" }}>
          <Link href="/shop" onClick={closeMenu} className="trail-link py-1 border-b border-ink/5 flex items-center justify-between">
            <span>SHOP</span>
            <span className="text-xs opacity-50">→</span>
          </Link>

          <Link href="/cart" onClick={closeMenu} className="trail-link py-1 border-b border-ink/5 flex items-center justify-between">
            <span>CART</span>
          ...
          {session?.user ? (
            <>
              <Link href="/account" onClick={closeMenu} className="trail-link py-1 border-b border-ink/5 flex items-center justify-between">
                <span>ACCOUNT</span>
                <span className="text-xs opacity-50">→</span>
              </Link>

              {session.user.isAdmin && (
                <Link href="/admin" onClick={closeMenu} className="trail-link py-1 border-b border-ink/5 flex items-center justify-between">
              ...
              <form action={logoutAction} onSubmit={closeMenu} className="mt-4">
                <button
                  type="submit"
                  className="w-full text-left py-2.5 px-4 rounded-full border border-oxblood/20 text-oxblood hover:bg-oxblood hover:text-cream transition-all text-sm font-semibold flex items-center justify-center gap-2 cursor-pointer"
                >
